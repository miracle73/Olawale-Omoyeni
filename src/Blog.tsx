        ...
        <div className="bg-black min-h-screen w-full pt-10 ">
            ...
            <div className='w-full flex flex-row justify-center items-center px-20 max-xl:px-10 max-sm:px-4'>
                <div className='h-14 max-xl:h-12 max-lg:h-10 max-md:h-8 w-1/2 max-lg:w-[65%] max-sm:w-3/4 max-xsm:w-[80%] max-xsxl:w-[92%] rounded-[25px] border-[0.03px] border-[#CBCBCB] bg-[#171717] flex flex-row justify-between items-center px-10 max-lg:px-7 max-md:px-5 max-sm:px-3'>
                    ...
                    <Link to="/blog"
                        className={`font-[400] text-[15px] max-xl:text-[13px] max-lg:text-[11px] max-md:text-[9px] max-xsm:text-[8px] font-[Poppins] text-white ${selectedItem === 'Blog' ? 'bg-[#9259F7] px-1 py-1  md:px-2  md:py-2 rounded-2xl max-sm:rounded-xl md:rounded-[17px]' : ''
                            }`}
            ...
            <p className='font-[600] font-[Poppins] text-[24px] text-white pt-10 px-20 max-xl:px-10 max-sm:px-4'>All blog post</p>

            <div className="flex w-full flex-row gap-10 flex-wrap justify-between items-center px-20 max-xl:px-10 max-sm:px-4 mt-10">
                {blogs.map((blog, index) => {
                    return (
                        <div className='w-[25%] pt-5' key={index}>
                            <div className="bg-myimage bg-no-repeat bg-contain w-full h-96 flex flex-row justify-end items-end pb-5">
                                <div className='bg-[#9358F7] h-24 w-24 rounded-[50px] flex flex-row justify-center items-center'>
                                    <img src={Arrow} />
                                </div>
                            </div>
                            <div className='py-2 '>
                                <div className='bg-[#F2F4F7] rounded-3xl h-fit w-fit p-3 flex flex-row justify-center items-center'>
                                    <p className='text-[#000000] font-[400] font-[Inter] text-[15px]'>{blog.name}</p>
                                </div>
                                <div className=' flex flex-row justify-start items-center gap-4 py-3'>
                                    <div className=' flex flex-row justify-start items-center gap-2'>
                                        ...
                                        <p className='font-[400] font-[Inter] text-[16px] text-white'>{blog.text1}</p>
                                    </div>
                                    <div className=' flex flex-row justify-start items-center gap-2'>
                                        <div className='h-[7px] w-[7px] rounded-md bg-[#FD853A]'></div>
                                        <p className='font-[400] font-[Inter] text-[16px] text-white'>{blog.text2}</p>
                                    </div>
                                </div>
                                <p className='text-[#FFFFFF] font-[400] font-[Poppins] text-[24px] pt-2'>{blog.text3}</p>
                            </div>
                        </div>
